...
if(strlen($password) > 20 || $password != $passwordAgain){
...
if( strpos(file_get_contents($fileLocation),$email) !== false) {
    header('Location: index.php?error=invalidEmail');
}

if(\strpos($username, ',') == true || \strpos($email, ',') == true || \strpos($password, ',') == true){
    header('Location: index.php?error=true');
}

if( strpos(file_get_contents($fileLocation),$username) !== false) {
    header('Location: index.php?error=invalidUsername');
}else{
    $data = "unconfirmed,$username,$email,$password";
    file_put_contents($fileLocation, $data.PHP_EOL , FILE_APPEND | LOCK_EX);

    $to      = $email;
    $subject = "Account confirmation";
    $message = "To confirm your account, click here:\nerewhon.xyz/game/confirmation.php?username=$username&email=$email";
