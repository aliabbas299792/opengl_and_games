...
Camera::Camera(int x, int y, float z){
    ...
    projection = glm::perspective(glm::radians(z), (float)windowWidth/windowHeight, 0.1f, 1000.0f);
...
void Camera::liveUpdate(){
    ...
    projection = glm::perspective(glm::radians(fov), (float)windowWidth/windowWidth, 0.1f, 1000.0f);
}