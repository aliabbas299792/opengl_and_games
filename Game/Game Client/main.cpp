...
const unsigned short REMOTE_UDP_PORT = 5001;
const unsigned short LOCAL_UDP_PORT = 5002;
...
int main(int argc, char **argv) { //will accept parameters from command line, the launcher will send parameters using this
	...
	*/
	...
	if (launcherKey == "Hg+Lb9B6Q/zrMduYEvPmuQ==") { //really weak check to verify that the launcher opened the program
		...
		gameNetwork* gameNetworkObj = new gameNetwork();
		...
		if (networkObject->login(username)) {
			gameNetworkObj->setData(networkObject->sessionID, LOCAL_UDP_PORT, REMOTE_UDP_PORT, REMOTE_IP); //sets essential data such as the session ID
